...
(function ($) {
    ...
    function getAttributesObject($select) {
        ...
        ParseableAttributes.forEach(function (item) {
            attrVal = $select[0].getAttribute(item);
            if (attrVal)
    ...
    class Selectpicker {
        constructor(element, options) {
            ...
            const that = this, id = this.$element.attr('id'), form = element.form;
            selectId++;
            ...
            if (this.options.container) {
                ...
                this.$element.on('hide' + EVENT_KEY, () => {
                    if (this.isVirtual()) {
                        const menuInner = this.$menuInner[0], emptyMenu = menuInner.firstChild.cloneNode(false);
            ...
            this.$newElement.on({
                'hide.bs.dropdown': (e) => {
                    this.$element.trigger('hide' + EVENT_KEY, e);
                },
                'hidden.bs.dropdown': (e) => {
                    this.$element.trigger('hidden' + EVENT_KEY, e);
                },
                'show.bs.dropdown': (e) => {
                    this.$element.trigger('show' + EVENT_KEY, e);
                },
                'shown.bs.dropdown': (e) => {
                    this.$element.trigger('shown' + EVENT_KEY, e);
                }
            });
            if (element.hasAttribute('required')) {
                this.$element.on('invalid' + EVENT_KEY, () => {
                    this.$button[0].classList.add('bs-invalid');
                    this.$element
                    ...
                    this.$button.on('blur' + EVENT_KEY, () => {
                        this.$element.trigger('focus').trigger('blur');
                        this.$button.off('blur' + EVENT_KEY);
                    });
                });
            }
            if (form) {
                $(form).on('reset' + EVENT_KEY, () => {
        ...
        createDropdown() {
            const showTick = (this.multiple || this.options.showTick) ? ' show-tick' : '', multiselectable = this.multiple ? ' aria-multiselectable="true"' : '';
            let inputGroup = '';
            const autofocus = this.autofocus ? ' autofocus' : '';
            if (version.major < 4 && this.$element.parent().hasClass('input-group')) {
                inputGroup = ' input-group-btn';
            }
            let drop, header = '', searchbox = '', actionsbox = '', donebutton = '', clearButton = '';
        ...
        createView(isSearching, setSize, refresh) {
            ...
            if (setSize) {
                if (refresh) {
                    ...
                    const element = that.$element[0], selectedIndex = (element.options[element.selectedIndex] || {})["liIndex"];
            ...
            function scroll(scrollTop, init) {
                ...
                if (init || positionIsDifferent || that.selectpicker.current.hasMore) {
                    ...
                    if ((init || isVirtual) && menuIsDifferent) {
                        const menuInner = that.$menuInner[0], firstChild = menuInner.firstChild, menuFragment = document.createDocumentFragment(), emptyMenu = firstChild.cloneNode(false);
                        ...
                        firstChild.appendChild(menuFragment);
                        if (isVirtual && that.sizeInfo.hasScrollBar) {
                            ...
                            if (init && menuInnerInnerWidth < that.sizeInfo.menuInnerInnerWidth && that.sizeInfo.totalMenuWidth > that.sizeInfo.selectWidth) {
                                ...
                                that.$menu[0].style.minWidth = String(0);
                ...
                if (!that.options.liveSearch) {
                    ...
                    const newActive = view.visibleElements[index];
        ...
        setPlaceholder() {
            ...
            if ((this.options.placeholder || this.options.allowClear) && !this.multiple) {
                ...
                if (titleNotAppended) {
                    ...
                    selectTitleOption = !selectedOption || (selectedIndex === firstSelectableIndex && !selectedOption.defaultSelected && this.$element.data('selected') === undefined);
                }
            ...
            if (data) {
                ...
                if (typeof data === 'function') {
                    data((data, more, totalItems) => {
                        ...
                        callback(builtData);
                        this.$element.trigger('fetched' + EVENT_KEY);
                    }, page, searchValue);
                }
        ...
        buildDataImpl(selectOptions, isData, type) {
            ...
            const addOption = (item, dataGetter, option_config) => {
                ...
                if (divider === true) {
                    ...
                    const option = ("option" in item ? item.option : item);
        ...
        buildData(data, type) {
            ...
            if (data !== false) {
                ...
                const elements = Array.from(this.$element[0].querySelectorAll('select > *' + selector));
                return this.buildDataImpl(elements, false, type);
            }
        }
        buildList(size, searching) {
            const that = this, selectData = searching ? this.selectpicker.search.data : this.selectpicker.main.data, mainElements = [];
            let widestOptionLength = 0;
            if ((that.options.showTick || that.multiple) && !elementTemplates._checkMark.parentNode) {
                elementTemplates._checkMark.className = this.options.iconBase + ' ' + that.options.tickIcon + ' check-mark';
            ...
            function buildElement(mainElements, item) {
                ...
                switch (item.type) {
                    ...
                    case 'option':
                        ...
                        break;
                    case 'optgroup-label':
                        liElement = generateOption.li(generateOption.label(item, that.options.iconBase), 'dropdown-header' + item.optgroupClass, item.optID === undefined ? undefined : String(item.optID));
                        break;
                }
                if (!item.element) {
        ...
        render(init) {
            const that = this, element = this.$element[0];
            let placeholderSelected = this.setPlaceholder() && element.selectedIndex === 0;
            const selectedOptions = this.getSelectedOptions(), selectedCount = selectedOptions.length, selectedValues = this.getSelectValues(selectedOptions), button = this.$button[0], buttonInner = button.querySelector('.filter-option-inner-inner'), multipleSeparator = document.createTextNode(this.options.multipleSeparator);
            ...
            if (this.options.selectedTextFormat === 'static') {
                ...
                if (!showCount) {
                    ...
                    const totalCount = this.$element[0].querySelectorAll('select > option' + optionSelector + ', optgroup' + optionSelector + ' option' + optionSelector).length, tr8nText = (typeof this.options.countSelectedText === 'function') ? this.options.countSelectedText(selectedCount, totalCount) : this.options.countSelectedText;
        ...
        setStyle(newStyle, status) {
            const button = this.$button[0], newElement = this.$newElement[0], style = this.options.style.trim();
            let buttonClass;
        ...
        liHeight(refresh) {
            ...
            const a = elementTemplates.a(), text = elementTemplates.span(), header = this.options.header && this.$menu.find('.' + classNames.POPOVERHEADER).length > 0 ? this.$menu.find('.' + classNames.POPOVERHEADER)[0].cloneNode(true) : null, search = this.options.liveSearch ? elementTemplates.div() : null, actions = this.options.actionsBox && this.multiple && this.$menu.find('.bs-actionsbox').length > 0 ? this.$menu.find('.bs-actionsbox')[0].cloneNode(true) : null, doneButton = this.options.doneButton && this.multiple && this.$menu.find('.bs-donebutton').length > 0 ? this.$menu.find('.bs-donebutton')[0].cloneNode(true) : null, firstOption = this.$element[0].options[0];
            ...
            if (this.selectpicker.current.data.length) {
                for (let i = 0; i < this.selectpicker.current.data.length; i++) {
                    ...
                    if (data.type === 'option' && $((_b = data.element) === null || _b === void 0 ? void 0 : _b.firstChild).css('display') !== 'none') {
                        li = data.element;
                        break;
                    }
                }
            ...
            const liHeight = li === null || li === void 0 ? void 0 : li.offsetHeight, dropdownHeaderHeight = dropdownHeader ? dropdownHeader.offsetHeight : 0, headerHeight = header ? header.offsetHeight : 0, searchHeight = search ? search.offsetHeight : 0, actionsHeight = actions ? actions.offsetHeight : 0, doneButtonHeight = doneButton ? doneButton.offsetHeight : 0, dividerHeight = $(divider).outerHeight(true), menuStyle = window.getComputedStyle(menu), menuWidth = menu.offsetWidth, menuPadding = {
        ...
        getSelectPosition() {
            const $window = $(window), pos = this.$newElement.offset(), $container = this.options.container === false ? null : $(this.options.container);
            let containerPos;
            if ($container !== null && $container.length && !$container.is('body')) {
        ...
        setMenuSize() {
            ...
            let menuInnerHeight = 0, menuHeight, divLength = 0, minHeight = "", _minHeight, maxHeight = "", menuInnerMinHeight = "", estimate, isDropup;
            ...
            if (this.options.width === 'auto') {
                requestAnimationFrame(function () {
                    ...
                    that.$element.on('loaded' + EVENT_KEY, function () {
                        that.liHeight();
                        that.setMenuSize();
                        const $selectClone = that.$newElement.clone().appendTo('body'), btnWidth = $selectClone.css('width', 'auto').children('button').outerWidth();
                        $selectClone.remove();
        ...
        selectPosition() {
            ...
            const that = this, $container = this.options.container === false ? null : $(this.options.container);
            let pos, containerPos, actualHeight;
            const getPlacement = function ($element) {
                var _b, _c;
                const containerPosition = {}, display = that.options.display || Dropdown.Default.display;
                (_b = that.$bsContainer) === null || _b === void 0 ? void 0 : _b.addClass($element.attr('class').replace(/form-control|fit-width/gi, '')).toggleClass(classNames.DROPUP, $element.hasClass(classNames.DROPUP));
                pos = $element.offset();
                if ($container !== null && !$container.is('body')) {
                    containerPos = $container.offset();
                ...
                (_c = that.$bsContainer) === null || _c === void 0 ? void 0 : _c.css(containerPosition);
            };
            this.$button.on('click.bs.dropdown.data-api', function () {
                ...
                if (that.options.container !== false) {
                    (_b = that.$bsContainer) === null || _b === void 0 ? void 0 : _b.appendTo(that.options.container).toggleClass(classNames.SHOW, !that.$button.hasClass(classNames.SHOW)).append(that.$menu);
                }
            });
            $(window)
            ...
            this.$element.on('hide' + EVENT_KEY, function () {
                var _b;
                that.$menu.data('height', String(that.$menu.height()));
                (_b = that.$bsContainer) === null || _b === void 0 ? void 0 : _b.detach();
            });
        }
        ...
        checkDisabled() {
            if (this.isDisabled()) {
                ...
                this.$button.addClass(classNames.DISABLED).attr('aria-disabled', String(true));
            }
            else {
                if (this.$button[0].classList.contains(classNames.DISABLED)) {
                    this.$newElement[0].classList.remove(classNames.DISABLED);
                    this.$button.removeClass(classNames.DISABLED).attr('aria-disabled', String(false));
                }
            }
        }
        clickListener() {
            const that = this, $document = $(document);
            ...
            function clearSelection() {
                if (that.multiple) {
                    ...
                    const element = that.$element[0], prevIndex = element.selectedIndex, prevOption = element.options[prevIndex], prevData = prevOption ? that.selectpicker.main.data[prevOption.liIndex] : false;
            ...
            function checkPopperExists() {
                if (that.dropdown._popper && that.dropdown._popper.state) {
                    setFocus();
                }
                else {
            ...
            this.$menuInner.on('mouseenter', 'li a', function () {
                const hoverLi = this.parentElement, position0 = that.isVirtual() ? that.selectpicker.view.position0 : 0, index = Array.prototype.indexOf.call(hoverLi.parentElement.children, hoverLi), hoverData = that.selectpicker.current.data[index + position0];
                that.focusItem(hoverLi, hoverData, true);
            });
            this.$menuInner.on('click', 'li a', (e, retainActive) => {
                const $this = $(e.target), element = that.$element[0], position0 = that.isVirtual() ? that.selectpicker.view.position0 : 0, clickedData = that.selectpicker.current.data[$this.parent().index() + position0], clickedElement = clickedData.element, prevIndex = element.selectedIndex, prevOption = element.options[prevIndex], prevData = prevOption ? that.selectpicker.main.data[prevOption.liIndex] : false;
                ...
                if (!that.isDisabled() && !$this.parent().hasClass(classNames.DISABLED)) {
                    const option = clickedData.option, state = option.selected, optgroupData = that.selectpicker.current.data.find(function (datum) {
                        return datum.type === 'optgroup-label' && datum.optID === clickedData.optID;
                    }), optgroup = optgroupData ? optgroupData.optgroup : undefined, dataGetter = optgroup instanceof Element ? getOptionData.fromOption : getOptionData.fromDataSource, optgroupOptions = optgroup && optgroup.children, maxOptions = that.options.maxOptions === false ? false : that.options.maxOptions, maxOptionsGrp = optgroup && parseInt(dataGetter(optgroup, 'maxOptions')) || false;
                    ...
                    if (!that.multiple || maxOptions === 1) {
                        ...
                        if (maxOptions !== false || maxOptionsGrp !== false) {
                            const maxReached = maxOptions !== false ? maxOptions < this.getSelectedOptions().length : false;
            ...
            this.$button
                ...
                if (tabindex !== null && e.originalEvent && e.originalEvent.isTrusted) {
                    this.setAttribute('tabindex', tabindex);
                    that.$element[0].setAttribute('tabindex', String(-1));
                    that.selectpicker.view.tabindex = tabindex;
                }
            })
                .on('blur' + EVENT_KEY, function (e) {
                if (that.selectpicker.view.tabindex !== undefined && e.originalEvent && e.originalEvent.isTrusted) {
                    that.$element[0].setAttribute('tabindex', String(that.selectpicker.view.tabindex));
        ...
        showNoResults(matchCount, searchValue) {
            if (!matchCount) {
                const r = elementTemplates.noResults();
                r.innerHTML = this.options.noneResultsText.replace('{0}', '"' + htmlEscape(searchValue) + '"');
                this.$menuInner[0].firstChild.appendChild(r);
            }
        }
        liveSearchListener() {
            const that = this;
            ...
            this.$searchbox.on('input propertychange', () => {
                const searchValue = this.$searchbox[0].value;
        ...
        keydown(e) {
            const $this = $(this), isToggle = $this.hasClass('dropdown-toggle'), $items = this.findLis();
            let index, isActive, liActive, activeLi, offset = 0, updateScroll = false;
            const downOnTab = e.which === keyCodes.TAB && !isToggle && !this.options.selectOnTab, isArrowKey = REGEXP_ARROW.indexOf(e.which) !== -1 || downOnTab, scrollTop = this.$menuInner[0].scrollTop, isVirtual = this.isVirtual(), position0 = isVirtual ? this.selectpicker.view.position0 : 0;
            if (e.which >= 112 && e.which <= 123)
                return;
            isActive = this.$menu.hasClass(classNames.SHOW);
            if (!isActive &&
                (isArrowKey ||
            ...
            if (e.which === keyCodes.ESCAPE && isActive) {
                e.preventDefault();
                this.$button.trigger('click.bs.dropdown.data-api').trigger('focus');
            }
            if (isArrowKey) {
                if (!$items.length)
                    return;
                ...
                if (e.which === keyCodes.ARROW_UP) {
                    ...
                    index++;
                    if (index + position0 >= this.selectpicker.view.canHighlight.length)
                        index = Number(this.selectpicker.view.firstHighlightIndex);
                    if (!this.selectpicker.view.canHighlight[index + position0]) {
                ...
                if (e.which === keyCodes.ARROW_UP) {
                    if (position0 === 0 && index === $items.length - 1) {
                        this.$menuInner[0].scrollTop = this.$menuInner[0].scrollHeight;
            ...
            if (isActive &&
                ...
                if (!this.options.liveSearch || e.which !== keyCodes.SPACE) {
                    this.$menuInner.find('.active a').trigger('click', true);
                    $this.trigger('focus');
                    if (!this.options.liveSearch) {
                        e.preventDefault();
        ...
        refresh() {
            const config = $.extend({}, this.options, getAttributesObject(this.$element), this.$element.data());
            this.options = config;
            if (this.options.source.data) {
                this.render();
                this.buildList();
    ...
    $(document)
        .on('keydown' + EVENT_KEY, '.bootstrap-select [' + Selector.DATA_TOGGLE + '], .bootstrap-select [role="listbox"], .bootstrap-select .bs-searchbox input', function (e) {
        this.keydown(e);
    })
